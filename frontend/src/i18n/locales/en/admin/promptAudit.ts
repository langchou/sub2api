export default {
  promptAudit: {
    ...
    pool: {
      title: 'Audit pool', description: 'Primary nodes fail over in order; one review node checks findings again. Probes run from the server network.',
    ...
    policy: {
      ...
      scanners: 'Qwen3Guard input-risk categories', workerCount: 'Worker count', reviewWorkerHint: 'An enabled review node fixes this at one worker.', queueCapacity: 'Persistent queue capacity', strategy: 'Node strategy', strategyHint: 'Primary nodes are tried in order; the review node is excluded from primary failover.',
    },
    saveBar: { enabled: 'Enable prompt audit', blocking: 'Synchronous blocking', reviewAsyncOnly: '8B review is available only in asynchronous audit mode.', blockingLatestTurnOnly: 'Only latest input and prior output', storePass: 'Store safe events', dirty: 'Unsaved changes', synced: 'Configuration synced' },
    blockingConfirm: {
    ...
    events: {
      ...
      primaryResult: 'Primary', reviewResult: 'Review', primaryDecision: 'Primary decision', reviewDecision: '8B review decision', reviewFailed: 'Review failed',
      ...
      guardReturn: 'Model audit return', primaryGuardReturn: 'Primary model return', reviewGuardReturn: '8B review model return',
      ...
      technical: {
        ...
        chunks: 'Chunks', latency: 'Latency', reviewEndpoint: 'Review endpoint', reviewLatency: 'Review latency', protocol: 'Protocol',
      },
    ...
    errors: {
      ...
      prompt_guard_requires_audit_enabled: 'Enable Prompt Audit before synchronous blocking.', prompt_audit_invalid_endpoint: 'The audit node configuration is invalid.', prompt_audit_invalid_endpoint_role: 'The audit node purpose is invalid.', prompt_audit_review_endpoint_duplicate: 'Configure at most one review node.', prompt_audit_review_async_only: '8B review is available only in asynchronous audit mode.', prompt_audit_review_single_worker: '8B review requires exactly one worker.', prompt_audit_endpoint_required: 'Enable at least one primary node before enabling Prompt Audit.', prompt_audit_groups_required: 'Select at least one group in selected-group mode.', prompt_audit_scanners_required: 'Enable at least one risk category.',
    },
  },
}
