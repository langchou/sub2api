export default {
  promptAudit: {
    ...
    policy: {
      ...
      scanners: 'Qwen3Guard input-risk categories', workerCount: 'Worker count', queueCapacity: 'Persistent queue capacity', strategy: 'Node strategy', strategyHint: 'Primary nodes are tried in order; the review node is excluded from primary failover.',
    },
    ...
    errors: {
      ...
      prompt_guard_requires_audit_enabled: 'Enable Prompt Audit before synchronous blocking.', prompt_audit_invalid_endpoint: 'The audit node configuration is invalid.', prompt_audit_invalid_endpoint_role: 'The audit node purpose is invalid.', prompt_audit_review_endpoint_duplicate: 'Configure at most one review node.', prompt_audit_review_async_only: '8B review is available only in asynchronous audit mode.', prompt_audit_endpoint_required: 'Enable at least one primary node before enabling Prompt Audit.', prompt_audit_groups_required: 'Select at least one group in selected-group mode.', prompt_audit_scanners_required: 'Enable at least one risk category.',
    },
  },
}
