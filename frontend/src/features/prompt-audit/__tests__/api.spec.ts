...
describe('Prompt Audit API', () => {
  ...
  it('sends a temporary probe token only in the request and never invents response credentials', async () => {
    ...
    const result = await promptAuditAPI.probeEndpoint({
      id: 'guard-1', name: 'Guard', role: 'primary', protocol: 'openai_compatible', base_url: 'http://127.0.0.1:8000', model: 'guard',
    ...
    expect(client.post).toHaveBeenCalledWith('/admin/prompt-audit/endpoints/probe', expect.objectContaining({ endpoint: expect.objectContaining({ role: 'primary', token: 'api-canary-secret' }) }))
